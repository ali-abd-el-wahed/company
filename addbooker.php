        ...
        &&  isset($_GET['cc']) && ($_GET['cc']!=""))
{
    ...
    $query="INSERT INTO `bookers`(`email`, `match_stadium_date`, `sit_cost`, `cc`) VALUES ('$email','$match','$sit','$cc')";
    //echo $query;
    ...
    {
        echo("a new booker has been inserted successfully");
    } 
}}
?>
<!DOCTYPE html>
  ...
  <div class="site-wrap">
      ...
      <div class="container">
        <div class="row align-items-center">
          <div class="col-lg-9 mx-auto text-center">
            <h1 class="text-white">ADD booker</h1>
    ...
    <div class="site-section">
      <div class="container">
        <div class="row">
          <div class="col-lg-7">
            <form action="addbooker.php" method="GET">
              <div class="form-group">
              <p>EMAIL :</p>
              <div class="form-group">
                <input type="email" class="form-control" placeholder="Email" name="email"required>
              </div>
              <p>MATCH - STADIUM (DATE) :</p>
              <div class="form-group">
        <select class="form-control" name="match">
            ...
            $query = "SELECT matches.oppenent, matches.stadium, matches.date FROM matches";
            $result = $mysqli->query($query);

            while ($row = $result->fetch_assoc()) {
                echo '<option value="' . $row['oppenent'] . ' - ' . $row['stadium'] . ' (' . $row['date'] . ')">' . $row['oppenent'] . ' - ' . $row['stadium'] . ' (' . $row['date'] . ')</option>';
            }
        ...
        <select class="form-control" name="sit">
            ...
            $query = "SELECT sit.class, sit.cost  FROM sit";
            $result = $mysqli->query($query);

            while ($row = $result->fetch_assoc()) {
                echo '<option value="' . $row['class'] . ' - ' . $row['cost'] . '">' . $row['class'] . ' - ' . $row['cost'] . ' </option>';
            }
        ...
        <div class="form-group">
         <input type="text" id="card_number" name="cc" placeholder="XXXX-XXXX-XXXX-XXXX" maxlength="16" required>
              ...
              <div class="form-group">
                ...
                <input type="submit" class="btn btn-primary py-3 px-5" value="ADD BOOKER">
              </div>
            </form>  
          </div>
